...
impl LightsMatrix {
    fn new(size: usize) -> Self {
        let d = (0..size).map(|_| false).collect::<Vec<_>>();
    ...
    fn set_square_range(&mut self, c1: (usize, usize), c2: (usize, usize), what_to_do: WhatToDo) {
        ...
        if bul {
            for x in c1.0..=c2.0 {
                for y in c1.1..=c2.1 {
                    match what_to_do {
                        WhatToDo::TurnOn => self.data[x][y] = true,
                        WhatToDo::TurnOff => self.data[x][y] = false,
                        WhatToDo::Toggle => self.data[x][y] = !self.data[x][y],
                        WhatToDo::Default => self.data[x][y] = self.data[x][y],
                    }
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn test1() {
        let result = process("turn on 0,0 through 999,999");
        assert_eq!(result, 1000 * 1000);
    }
    #[test]
    fn test2() {
        let result = process("toggle 0,0 through 999,0");
